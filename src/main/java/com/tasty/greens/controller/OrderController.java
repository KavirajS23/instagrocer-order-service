...
public class OrderController {
	...
	public ResponseEntity<Object> fetchOrderById(@PathVariable(name = "orderId") Long orderId) {
		OrderResponse orderResponse = orderService.fetchOrderById(orderId);
		if(orderResponse != null) {
			return new ResponseEntity<>(orderResponse, HttpStatus.OK);
		}
		return new ResponseEntity<>("No order found for orderId: "+orderId, HttpStatus.NOT_FOUND);
	}
}
